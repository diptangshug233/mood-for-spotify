...
const normalizeCoord = (coord, squareSize, circleSize) =>
  Math.min(Math.max(0 + circleSize / 2, coord), squareSize - circleSize / 2) /
  squareSize;
...
const MoodSelector = (props) => {
  ...
  const squareSize = Math.min(350, 0.65 * windowWidth);
  ...
  useEffect(() => {
    const mouseUpHandler = () => {
      if (isPointerDown) {
        ...
        props.setProperties(audioProperties);
      }
    };
